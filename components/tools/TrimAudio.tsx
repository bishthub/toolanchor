...
export default function TrimAudio({ initialFiles }: { initialFiles?: File[] }) {
  ...
  const outName = useRef("clip.mp3");

  function pick(f: File | null) {
    setError(null); setUrl(null); setFile(f);
    if (f) { setSrcUrl(URL.createObjectURL(f)); setStart("0"); setEnd(""); }
    else setSrcUrl(null);
  }

  useEffect(() => {
    const f = initialFiles?.find((x) => x.type.startsWith("audio/"));
    if (f) pick(f);
  ...
  async function run() {
    ...
    try {
      ...
      const type = ext === ".wav" ? "audio/wav" : ext === ".ogg" ? "audio/ogg" : ext === ".m4a" ? "audio/mp4" : "audio/mpeg";
      setUrl(URL.createObjectURL(new Blob([data.slice().buffer], { type })));
      setStatus("");
    } catch (err) {
      console.error(err);
      setError("Could not trim this audio. Try a different file.");
    } finally {
  ...
  return (
    <div>
      <div className="field">
        ...
        <input type="file" accept="audio/*" onChange={(e) => pick(e.target.files?.[0] || null)} className="input" disabled={busy} />
      </div>
      ...
      {dur > 0 && (
        <div className="row">
          ...
          <div className="field">
            <label>End (s) — track is {dur.toFixed(1)}s long</label>
        ...
        <div style={{ marginTop: 16 }}>
          ...
          <SendToTool
            ...
            getFile={async () => {
              ...
              return new File([blob], outName.current, { type: blob.type || "audio/mpeg" });
            }}
          />
        </div>
      )}

      <p className="privacy-note">🔒 Runs in your browser via ffmpeg.wasm — your audio is never uploaded. Trimming copies the stream, so there’s no quality loss.</p>
    </div>
  );
}
